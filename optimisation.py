#!/usr/bin/python
import subprocess, sys, os, math, datetime, shutil, getpass, platform
                        ...
                        if len(filesplit) > 1: # there is an extension
                            if filesplit[-1] == "mop": 
                                ...
                                if backup_energy(directory, filename, log) == True: 
                                    ...
                                    job = Popen([setup.exe,  join(directory, filename)], stdout=PIPE, startupinfo=startupinfo)
                                    ...
                                    if isJobFinished(directory, filename) == True:
                                        ...
                                        log.write("Warning: Exit anormally. Look at the output file of the input [%s]" % join(directory, filename))
                                else:
